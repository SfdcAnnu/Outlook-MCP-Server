...
export function requireBearer(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!auth || !auth.startsWith('Bearer ')) {
    res.status(401)
      .set(
        'WWW-Authenticate',
        `Bearer realm="outlook-mcp", resource_metadata="${process.env.BASE_URL}/.well-known/oauth-protected-resource"`
      )
      .json({
        error: 'unauthorized',
        message: 'Bearer token required. Connect via the Outlook MCP connector in Claude Settings.'
      })
    return
  }
